...
@Directive({
  selector: '[appAnswerNavClick]'
})
export class AnswerNavClickDirective {
  ...
  ngOnInit() {
    if(this.el.nativeElement.className=='active'){
      this.router.navigate(['/answer'+this.el.nativeElement.firstElementChild.getAttribute('routerLink').substring(1)+'']);
    }
  }
}
